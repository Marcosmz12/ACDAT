...
function appendToDisplay(value: string): void {
    ...
    if (['+', '-', '*', '/', '^'].includes(value)) {
        ...
        if (currentInput !== '0' && currentInput !== '') {
            previousInput = currentInput;
            operator = value;
            calculate(); // Ejecuta inmediatamente
        }
    } else {
        update = true;
...
function calcularRaizCuadrada(num: number): number | undefined {
    if (num < 0) {
        alert('Error: No se puede calcular la raíz cuadrada de un número negativo');
        console.error('Error: Raíz cuadrada de número negativo');
        return undefined;  // Indicamos que no hay resultado válido
    }
